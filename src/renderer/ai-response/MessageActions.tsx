...
export function MessageActions({ markdown, plainText, onRegenerate, onDelete, onCopyPair }: {
  ...
  return (
    <div className="message-actions assistant">
      <button className="message-action" onClick={() => actions.copyText(markdown)} aria-label="复制" title="复制"><Copy size={14} /></button>
      ...
      <span className="message-more" ref={ref}>
        ...
        {menuOpen && <div className="message-menu" role="menu">
          <button role="menuitem" onClick={() => { actions.copyText(markdown); setMenuOpen(false) }}><Copy size={14} />复制为 Markdown</button>
